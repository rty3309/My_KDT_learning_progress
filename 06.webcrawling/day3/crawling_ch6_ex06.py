...
def login(driver, user_id, pwd):
    ...
    # 아이디 입력
    login_id = driver.find_elements(By.ID, 'user_email')
    login_id.send_keys(user_id)

    # 비밀번호 입력
    login_pwd = driver.find_elements(By.ID, 'user_passoword')
    login_pwd.send_keys(passwd)
...
def get_review_score(driver):
    ...
    for company_name in company_dict.keys():
        score_title_list = []    # 평점 이름(전체평점, 복지, ...)
        score_list = []
    ...
    # company_score_df.to_excel('company_score.xlsx', index=False)
...
def main():
    ...
    #login(driver, user_id, passwd)    # login 하지 않아도 크롤링 가능
