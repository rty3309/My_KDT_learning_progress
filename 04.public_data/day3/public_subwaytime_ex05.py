...
with open('subwaytime.csv', encoding='utf-8-sig') as f:
    ...
    for i in range(4,27):    # x축의 tick을 4,5,6,...,23,0,1,2시 로 표시하기 위함(24 % 24=0)
        n=i % 24    # 18시, 19시 이렇게 표시하기 위해서 쓰는 for문
        xtick_list.append(str(n))

    for row in data:
        row[4:] =map(int,row[4:])
        for j in range(23):
            a = row[j*2 + 4]    # j=0: data[0*2+4] -> data[4]의 값을 max[0]에 저장하기 위함
            if a > max[j]:
                max[j] = a
